...
from app.models.entities import FeatureReport
from app.services.records.adapters.base import LegacyRecordAdapter


class ReportAdapter(LegacyRecordAdapter):
    record_type = "report"
    audit_entidad_tipo = "feature_report"

    def get_entity(self, db: Session, record_id: uuid.UUID) -> FeatureReport | None:
        return db.get(FeatureReport, record_id)
    ...
    def get(self, db: Session, record_id: uuid.UUID) -> RecordDTO | None:
        ...
        return RecordDTO(
            ...
            storage="legacy",
            titulo=row.descripcion[:120] if row.descripcion else f"Reporte {row.tipo}",
    ...
    def list(
        ...
        *,
        ...
        if parent_id is not None:
            stmt = stmt.where(FeatureReport.feature_id == parent_id)
        return [dto for r in db.scalars(stmt) if (dto := self.get(db, r.id))]
